...
class DataPersistenceThread(threading.Thread):
    """Background thread for saving data using pickle"""
    ...
    def run(self):
        """Continuously save data from queue"""
        while self.running:
            try:
                # Check for data to save (non-blocking)
                data_type, data = self.save_queue.get(timeout=1)
                self._save_data(data_type, data)
            except queue.Empty:
                continue
            except Exception as e:
                print(f"Error in persistence thread: {e}")
    
    def _save_data(self, data_type, data):
        """Save specific data type to pickle file"""
        try:
            if data_type == 'students':
                with open("students.pkl", "wb") as f:
                    pickle.dump(data, f)
                print("[Background] Saved students.pkl")
            elif data_type == 'courses':
                with open("courses.pkl", "wb") as f:
                    pickle.dump(data, f)
                print("[Background] Saved courses.pkl")
            elif data_type == 'marks':
                with open("marks.pkl", "wb") as f:
                    pickle.dump(data, f)
                print("[Background] Saved marks.pkl")
        except Exception as e:
...
def main_curses(stdscr):
    ...
    # Create queue and start background persistence thread
    save_queue = queue.Queue()
    persistence_thread = DataPersistenceThread(save_queue)
    persistence_thread.start()
    
    # Set the queue in input module
    inp.set_save_queue(save_queue)
